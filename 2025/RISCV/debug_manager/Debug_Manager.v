...
module debug_manager #(
    ...
    input       [10-1:0]            i_D_control_bits,
    input       [N_BITS_REG-1 : 0]  i_D_rs1,
    input       [N_BITS_REG-1 : 0]  i_D_rs2,
    input       [N_BITS_REG-1 : 0]  i_D_rd,
    ...
    localparam [2:0] SENDING    = 3'b101; // Envaindo por UART
    localparam [2:0] SEND_START = 3'b110; // Iniciando el envio
    ...
    interface_tx u_interface_tx (
        ...
        .i_D_control_bits(i_D_control_bits),//10
        .i_D_rs1(i_D_rs1),//5
        .i_D_rs2(i_D_rs2),//5
        .i_D_rd(i_D_rd),//5
    ...
    always @(*) begin
        ...
        case (current_state)
            ...
            REC: begin
                if ((byte_counter == 3'd3 && i_rx_done && (current_addr + 1'b1) == MAX_INSTRUCTIONS) || halt_detected)
                    next_state = WAIT_CMD;
            end

            EXEC_STEP: begin
                next_state = WAIT_CMD; // Pasa a enviar tras el pulso de step
            end

            WAIT_HALT: begin
                if (i_halt_risc) next_state = SEND_START; // Espera señal física de Halt
            end
